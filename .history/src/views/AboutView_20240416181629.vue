<template>
  <div id="memory-game">
    <div class="cards">
      <div class="row">
        <div v-for="(card, index) in topCards" :key="index" class="card"
          :class="{ flipped: card.flipped, matched: card.matched, hover: cardHovered === index }"
          @click="flipCard(index)" @mouseover="cardHovered = index" @mouseleave="cardHovered = null">
          <div class="card-inner">
            <div class="card-front"
              style="background-image: url('https://img0.baidu.com/it/u=3318691808,1333308276&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=667')">
            </div>
            <div class="card-back">
      ...
      <div class="row">
        <div v-for="(card, index) in bottomCards" :key="index" class="card"
          :class="{ flipped: card.flipped, matched: card.matched, hover: cardHovered === index + totalPairs }"
          @click="flipCard(index + totalPairs)" @mouseover="cardHovered = index + totalPairs"
    ...
    <div class="timer" v-if="gameStarted && !gameOver">用时：{{ time }}</div>
    <div class="message" v-if="!gameStarted">
      <p>点击按钮后开始游戏</p>
    </div>
    <t-dialog theme="warning" header="警告" :body="'请先点击开始按钮！'" :visible.sync="showStartGameModal" @confirm="closeModal1"
      :cancelBtn="null" />
    <t-dialog theme="success" header="恭喜通关" :body="`完成游戏用时：${time}秒!`" :visible.sync="showModal" @confirm="closeModal2"
      :cancelBtn="null" />
...
export default {
  ...
  data() {
    return {
      ...
      cardHovered: null // 用于跟踪鼠标悬停的卡片索引
    };
  },
  computed: {
    topCards() {
  ...
  methods: {
    ...
    flipCard(index) {
      if (!this.gameStarted) {
        this.showStartGameModal = true; // 显示开始游戏 modal
        return; // 不执行任何操作
      }
    ...
    hideStartGameModal() {
      this.showStartGameModal = false; // 关闭开始游戏 modal
    },
    checkMatch() {
