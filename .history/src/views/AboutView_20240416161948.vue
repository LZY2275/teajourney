...
export default {
  data() {
    return {
      ...
      waitingForMatch: false // New state to track if waiting for match
    };
  },
  computed: {
    topCards() {
  ...
  methods: {
    ...
    initializeGame() {
      ...
      values.forEach((value, index) => {
        this.cards.push({
          value: value,
          flipped: true, // Initially show cards
          matched: false
        });
      });
    ...
    flipCard(index) {
      if (!this.gameStarted || this.gameOver || this.waitingForMatch) return; // Ignore clicks if game hasn't started, is over, or waiting for match
      if (this.flippedCards.length < 2 && !this.cards[index].flipped && !this.cards[index].matched) {
        this.cards[index].flipped = true;
        this.flippedCards.push(index);

        if (this.flippedCards.length === 2) {
          // Set a waiting flag to prevent flipping other cards
    ...
    checkMatch() {
      ...
      // Reset waiting flag after match check
