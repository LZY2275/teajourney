        ...
        myChart.setOption({
            ...
            tooltip: {
                ...
                formatter: function (params) {
                    var tooltipHtml = params[0].axisValueLabel + '<br/>';var guangdongFirst = true;
                    params.forEach(function (item) {
    var value;

    if (item.seriesName === '广东' && guangdongFirst) {
        value = item.value.toFixed(3); // 对于广东省的第一个数据显示小数点后三位
        guangdongFirst = false; // 将标志设置为 false，表示已处理过广东省的第一个数据
    } else if (item.seriesName === '广东') {
        value = item.value.toFixed(2); // 对于广东省的其他数据显示小数点后两位
