<template>
  ...
  <div>
    <t-guide :current.sync="sale_guide_current" :steps="sale_steps" @finish="handleFinish"
    ...
    <div style="display: flex;padding: 0 10px 10px 10px;">
      <div id="silklable" style="padding: 10px 9px;"></div>
...
export default {
  ...
  methods: {
    handleResize() {
      ...
      d3.select('#silklable').selectAll('*').remove();
    ...
    drawLable() {
      ...
      var tea_Type = [this.$t('绿茶'), this.$t('白茶'), this.$t('黄茶'), this.$t('乌龙茶'), this.$t('红茶'), this.$t('黑茶')];
      //定义6个颜色比例尺，用于5条折线着色
      ...
      var selectAllText = this.$t('选择所有');
      d3.select("#silklable")
        .html('<p id="select_all" style="padding:10px;font-weight:bold;cursor:pointer"></p>')
      // 将文本插入到选中的元素中
      d3.select("#select_all")
        .text(selectAllText);
      ...
      for (var j = 0; j < _data.length; j++) {
        ...
        var rects = svg.append("g")
          ...
          .on('mouseover', function (d, i) {
            ...
            tooltip.call(callout, that.$t(tea_type) + ': ' + i.value +  that.$t(_data[0].danwei))
  ...
  computed:{
      ...
      return [
    {
          ...
          title: this.$t('丝绸图'),
          body: this.$t('显示各类茶近五年数据的详细情况，颜色代表茶的类别'),
          placement: 'bottom-left',
        },
        {
          element: '#circle_sel_0',
          title: this.$t('选择图例'),
          body: this.$t('点击小圆圈，详细查看某种茶的数据'),
          placement: 'right',
        },
        {
          element: '#select_all',
          title: this.$t('选择所有'),
          body: this.$t('点击此处，选择查看所有图例'),
          placement: 'right',
        },
        {
          element: '.t-tabs',
          title: this.$t('选项卡'),
          body: this.$t('可以选择要查看的数据类别'),
          placement: 'bottom',
        },
        ]
      }
